    ...
    var expand = (ast, options = {}) => {
      ...
      const walk = (node2, parent = {}) => {
        ...
        if (node2.invalid || node2.dollar) {
          q.push(append2(q.pop(), stringify2(node2, options)));
          return;
        }
...
var require_parse = __commonJS({
  "node_modules/braces/lib/parse.js"(exports2, module2) {
    ...
    var parse2 = (input, options = {}) => {
      ...
      const push = (node2) => {
        if (node2.type === "text" && prev2.type === "dot") {
          prev2.type = "text";
        }
        if (prev2 && prev2.type === "text" && node2.type === "text") {
          prev2.value += node2.value;
          return;
        }
      ...
      while (index < length2) {
        ...
        if (value === CHAR_LEFT_SQUARE_BRACKET) {
          brackets++;
          let next2;
          while (index < length2 && (next2 = advance())) {
            value += next2;
        ...
        if (value === CHAR_LEFT_CURLY_BRACE) {
          depth++;
          const dollar = prev2.value && prev2.value.slice(-1) === "$" || block.dollar === true;
          const brace = {
            type: "brace",
            open: true,
            close: false,
        ...
        if (value === CHAR_DOT && depth > 0 && block.commas === 0) {
          ...
          if (prev2.type === "range") {
            ...
            before.value += prev2.value + value;
            prev2 = before;
            block.ranges--;
            continue;
          }
    ...
    var scan = (input, options) => {
      ...
      while (index < length2) {
        ...
        if (code === CHAR_FORWARD_SLASH) {
          ...
          if (prev2 === CHAR_DOT && index === start + 1) {
            start += 2;
            continue;
          }
          lastIndex = index + 1;
        ...
        if (code === CHAR_ASTERISK) {
          if (prev2 === CHAR_ASTERISK) isGlobstar = token2.isGlobstar = true;
          isGlob = token2.isGlob = true;
          finished = true;
          if (scanToEnd === true) {
            continue;
      ...
      while (!eos()) {
        ...
        if (prev2.type === "star") {
          ...
          if (prior.type === "slash" && prior.prev.type !== "bos" && rest[0] === "/") {
            ...
            prior.output = `(?:${prior.output}`;
            prev2.type = "globstar";
            prev2.output = `${globstar(opts)}${SLASH_LITERAL}|${SLASH_LITERAL}${end})`;
            prev2.value += value;
            state.output += prior.output + prev2.output;
            state.globstar = true;
...
var EXTENSION_NAME = "codecanvas";
var BACKGROUND_VER = "codecanvas.ver";
var VERSION = "1.0.0";
var ENCODING = "utf8";
var TOUCH_FILE_PATH = import_path.default.join((0, import_os.tmpdir)(), "codecanvas.touch");
...
var AbsPatchFile = class {
  ...
  async getContent() {
    return import_fs.default.promises.readFile(this.filePath, ENCODING);
  }
  async saveContentTo(targetFilePath, content) {
    try {
      if (import_fs.default.existsSync(targetFilePath)) {
        await import_fs.default.promises.access(targetFilePath, import_fs.constants.W_OK);
      }
      await import_fs.default.promises.writeFile(targetFilePath, content, ENCODING);
      return true;
    } catch (e) {
      const retry = "Retry with Admin/Sudo";
      const result = await vscode2.window.showErrorMessage(
      ...
      const tempFilePath = import_path2.default.join((0, import_os2.tmpdir)(), `codecanvas-temp-${(0, import_crypto.randomUUID)()}.temp`);
      await import_fs.default.promises.writeFile(tempFilePath, content, ENCODING);
      try {
        ...
        vscode2.window.showErrorMessage(`Failed to write with elevated privileges: ${e2.message}`);
        return false;
      } finally {
        await import_fs.default.promises.rm(tempFilePath, { force: true });
      }
    }
  }
  async write(content) {
  ...
  async restore() {
    try {
      ...
      vscode2.window.showErrorMessage(`Failed to restore file: ${e.message}`);
      return false;
    }
  }
};
var JsPatchFile = class extends AbsPatchFile {
...
// codecanvas-background-start ${BACKGROUND_VER}.${VERSION}`,
      ...
      vscode2.window.showErrorMessage(`Failed to apply patches to JS file: ${e.message}`);
      return false;
    }
  }
...
// src/background/PatchGenerator.ts
...
// src/background/PatchGenerator.ts
var EditorPatchGeneratorConfig = class {
  useFront = true;
  style = {};
  styles = [];
  images = [];
...
var FullscreenPatchGeneratorConfig = class {
  images = [];
  opacity = 0.1;
...
function css(template, ...args) {
  return template.reduce((prev2, curr, i) => {
    let arg = args[i];
    ...
    return prev2 + curr + (arg ?? "");
  }, "");
}
var AbsPatchGenerator = class {
  config;
  imageRequired = true;
  // Must have images
  constructor(config) {
    ...
    return images.map((imageUrl) => {
      try {
        if (!imageUrl.startsWith("file://")) {
          imageUrl = (0, import_url.pathToFileURL)(imageUrl).href;
        }
        const url = imageUrl.replace("file://", "vscode-file://vscode-app");
        return vscode3.Uri.parse(url).toString();
      } catch {
        return "";
      }
    });
  ...
  getImagesFromFolders(folders = []) {
    try {
      ...
      return import_fast_glob.default.sync(
        folders.map((f) => f.replace(/\\/g, "/").replace(/\/+$/, "")).map((f) => `${f}/**/*.@(${types.join("|")})`),
...
`;
  ...
  create() {
    ...
    return [
      ...
      script
    ].filter((n) => !!n.length).map((n) => _.withIIFE(n)).join(";");
  }
};
var WithoutImagesPatchGenerator = class extends AbsPatchGenerator {
  constructor() {
    super({ images: [] });
  }
  imageRequired = false;
};
var ChecksumsPatchGenerator = class extends WithoutImagesPatchGenerator {
  Translations = [
    // en, default
    "installation appears to be corrupt. Please reinstall.",
    // cs
    "je pravd\u011Bpodobn\u011B po\u0161kozen\xE1. Prove\u010Fte pros\xEDm p\u0159einstalaci.",
    // de
    "Installation ist offenbar besch\xE4digt. F\xFChren Sie eine Neuinstallation durch.",
    // es
    "parece estar da\xF1ada. Vuelva a instalar.",
    // fr
    "semble \xEAtre endommag\xE9e. Effectuez une r\xE9installation.",
    // it
    ...
    // pl
    "prawdopodobnie jest uszkodzona. Spr\xF3buj zainstalowa\u0107 ponownie.",
    // pt-BR
    "parece estar corrompida. Reinstale-o.",
    // qps-ploc
    "\xEF\xF1st\xE6ll\xE6t\xEF\xF8\xF1 \xE6pp\xEB\xE6rs t\xF8 \xFE\xEB \xE7\xF8rr\xB5pt. Pl\xEB\xE6s\xEB r\xEB\xEF\xF1st\xE6ll.",
    // ru
    "\u043F\u043E\u0432\u0440\u0435\u0436\u0434\u0435\u043D\u0430. \u041F\u043E\u0432\u0442\u043E\u0440\u0438\u0442\u0435 \u0443\u0441\u0442\u0430\u043D\u043E\u0432\u043A\u0443.",
    // tr
    "y\xFCklemeniz bozuk gibi g\xF6r\xFCn\xFCyor. L\xFCtfen yeniden y\xFCkleyin.",
    // zh-hans
    "\u5B89\u88C5\u4F3C\u4E4E\u635F\u574F\u3002\u8BF7\u91CD\u65B0\u5B89\u88C5\u3002",
    // zh-hant
    "\u5B89\u88DD\u4F3C\u4E4E\u5DF2\u640D\u6BC0\u3002\u8ACB\u91CD\u65B0\u5B89\u88DD\u3002"
...
var ThemePatchGenerator = class _ThemePatchGenerator extends WithoutImagesPatchGenerator {
...
var EditorPatchGenerator = class extends AbsPatchGenerator {
  ...
  getStyleByOptions(style, useFront) {
    ...
    return Object.entries(style).filter(([key]) => !excludeKeys.includes(key)).map(([key, value]) => `${key}: ${value};`).join("");
  }
  get imageStyles() {
      ...
      return this.getStyleByOptions(
        {
          ...style,
          ...styles && styles[index] || {},
          // Handle styles[index] being undefined
...
var FullscreenPatchGenerator = class extends AbsPatchGenerator {
...
var SidebarPatchGenerator = class extends FullscreenPatchGenerator {
...
var PatchGenerator = class {
  static create(options) {
    ...
    const script = scriptParts.filter((n) => !!n.length).map((n) => _.withIIFE(n)).join(";");
    return script;
  }
};
...
var VscodePath = class {
  extRoot;
  workbenchPath;
  jsPath;
  cssPath;
  constructor() {
    const appRoot = vscode4.env.appRoot;
    const extensionId = "tupynambalucasdev.codecanvas";
    const extension = vscode4.extensions.getExtension(extensionId);
    this.extRoot = extension ? extension.extensionPath : "";
    this.workbenchPath = import_path3.default.join(appRoot, "out", "vs", "workbench", "workbench.desktop.main.js");
    if (!import_fs2.default.existsSync(this.workbenchPath)) {
      this.workbenchPath = import_path3.default.join(appRoot, "vs", "workbench", "workbench.desktop.main.js");
    }
    this.jsPath = this.workbenchPath;
    this.cssPath = import_path3.default.join(appRoot, "out", "vs", "workbench", "workbench.desktop.main.css");
  }
};
...
var vsHelp = new class {
  ...
  reload(options = {}) {
    ...
    vscode5.window.showInformationMessage(message, btnReload).then(async (selection) => {
...
var BackgroundManager = class {
  ...
  jsFile;
  get config() {
    const cfg = vscode6.workspace.getConfiguration(EXTENSION_NAME);
    const ui = cfg.get("ui") || {};
    const legacyConfig = {
  ...
  async checkFirstload() {
    const firstLoad = !fs3.existsSync(TOUCH_FILE_PATH);
    if (firstLoad) {
      await fs3.promises.writeFile(TOUCH_FILE_PATH, vscodePath.jsPath, ENCODING);
      return true;
    }
    return false;
  }
  ...
  async install() {
    ...
    if (success) {
      vscode6.window.showInformationMessage("CodeCanvas: Patch applied successfully.");
    } else {
      vscode6.window.showErrorMessage("CodeCanvas: Failed to apply patch.");
    }
    return success;
  }
  async uninstall() {
    ...
    if (success) {
      vscode6.window.showInformationMessage("CodeCanvas: Patch uninstalled successfully.");
    } else {
      vscode6.window.showErrorMessage("CodeCanvas: Failed to uninstall patch.");
    }
    return success;
  }
  async hasInstalled() {
