...
var VscodePath = class {
  ...
  cssPath;
  constructor() {
    const appRoot = import_path3.default.dirname(require.main.filename);
    this.extRoot = vscode4.extensions.getExtension("tupyn.codecanvas").extensionPath;
    if (process.platform === "win32") {
      this.workbenchPath = import_path3.default.join(appRoot, "vs", "workbench", "workbench.desktop.main.js");
    } else if (process.platform === "darwin") {
      this.workbenchPath = import_path3.default.join(
        appRoot,
        "..",
        "Resources",
        "app",
        "out",
        // Assuming it's in out folder, verify if necessary
        "vs",
        "workbench",
    ...
    this.cssPath = import_path3.default.join(appRoot, "vs", "workbench", "workbench.desktop.main.css");
  }
};
...
var BackgroundManager = class {
  ...
  async checkFirstload() {
    const firstLoad = !fs2.existsSync(TOUCH_FILE_PATH);
    if (firstLoad) {
      await fs2.promises.writeFile(TOUCH_FILE_PATH, vscodePath.jsPath, ENCODING);
      return true;
    }
    return false;
  }
